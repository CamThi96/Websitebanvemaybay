<div class="item">
    ...
    <?php
		echo form_open_multipart(base_url().$this->uri->uri_string());
			$this->table->add_row(array('data'=>'Tên Sản Phẩm', 'width'=>150), array('data'=>form_input('title_vi'), 'width'=>600));
			//$this->table->add_row(array('data'=>'Tên Sản Phẩm Tiếng Anh', 'width'=>150), array('data'=>form_input('title_en'), 'width'=>600));
			foreach($main as $val){
			...
			$this->table->add_row('Thuộc Danh Mục', form_dropdown('product_type', $option));
			//$this->table->add_row('Giá Sản Phẩm', form_input('price'));
			$this->table->add_row('Hình Đại Diện', form_upload('user_file'));
			$this->table->add_row('Chi Tiết Sản Phẩm', form_textarea('content_vi'));
			//$this->table->add_row('Chi Tiết Sản Phẩm Tiếng Anh', form_textarea('content_en'));
