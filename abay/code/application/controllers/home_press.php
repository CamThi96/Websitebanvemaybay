<?php
	...
	class Home_Press extends Home_Default{
		/*function __Construct(){
			parent::__Construct();
			parent::_Set_Default_Right();
			parent::_Add_Title('Báo Chí Nói Về Chúng Tôi');
		}
		
		function index(){
			$this->load->model('mdpost');
			$data['content']=$this->mdpost->GetPostByCode('press');
			$data['content']=$data['content']['content_vi'];
			parent::_Set_Default_Left($this->my_layout->input('front_end/content/press', $data));
			//$this->my_layout->set_data('content_for_website', $this->my_layout->get_data('c'
		...
		function _Check_news(){
			if($this->uri->segment(2)==''){
				...
				$data['news']=$this->mdnews->GetNews($this->uri->segment(2), 'press');
				if($data['news']==false){
					$this->_View_All();
				}else{
		...
		function _View_All(){
			...
			parent::_Set_Default_Left($this->my_layout->input('front_end/content/all_press',$data));
		}
	}
?>