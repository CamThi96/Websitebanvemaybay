<?php
	...
	class Home_recruitment extends Home_Default{
		function __Construct(){
			parent::__Construct();
			$this->load->library('pagination');
			$this->load->model('mdnews');
			parent::_Add_Title('Tuyển Dụng');
		}
		...
		function _Check_news(){
			if($this->uri->segment(2)==''){
				...
				$data['news']=$this->mdnews->GetNews($this->uri->segment(2), 'recruitment');
				if($data['news']==false){
					$this->_View_All();
				}else{
		...
		function _View_All(){
			...
			parent::_Set_Default_Left($this->my_layout->input('front_end/content/all_recruitment',$data));
		}
	}
?>