<?php
	...
	class Tips_Default extends right_Default{
		var $obj;
		function __Construct(){
			parent::__Construct();
			$this->obj=&get_instance();
			$this->obj->load->model('mdnews');
			$this->my_layout->set_data('content_for_title_website', 'Thủ Thuật - '.$this->my_layout->get_data('content_for_title_website'));
		}
		...
		function Show_News(){
			if($this->uri->segment(2)==''){
				...
				$data['news']=$this->obj->mdnews->GetNews($this->uri->segment(2), 'tips');
				if($data['news']==false){
					$this->Show_all_news();
				}else{
		...
		function Show_all_news(){
			$data['news']=$this->obj->mdnews->GetNewsByType('tips', 99999, 0);
			$data['title']='Thủ Thuật';
